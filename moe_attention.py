...
class AggregationModule(nn.Module):
    def __init__(self, num_classes, num_experts=3):
        ...
        # Attention mechanism to weigh expert outputs
        self.attention_layer = nn.Sequential(
            nn.Linear(num_experts * num_classes, num_experts),
            nn.Softmax(dim=1)
        )
        
        # Feature transformation and aggregation with BatchNorm
    ...
    def forward(self, expert_outputs):
        ...
        attention_weights = self.attention_layer(concat_outputs)
        
        # Apply attention weights to the expert outputs
        weighted_outputs = concat_outputs * attention_weights.repeat(1, NUM_CLASSES)
        
        # Forward pass through the aggregation module
        aggregated_output = self.fc(weighted_outputs)
...
def train_aggregation_module(expert_models, aggregation_module, dataloader, num_epochs=30, learning_rate=1e-3):
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(aggregation_module.parameters(), lr=learning_rate, weight_decay=1e-1)
...
def main():
    ...
    evaluate_mixture_of_experts(expert_models, aggregation_module, val_loader, report_filename="mixture_classification_attention_report.txt")
    
    # Save the aggregation module
    save_model(aggregation_module, 'mixture_classification_attention_module.pth')
