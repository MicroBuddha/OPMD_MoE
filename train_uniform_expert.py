...
def evaluate(model, dataloader, report_filename="uniform_classification_report.txt"):
    model.eval()
    correct_predictions = 0
    total = 0
    all_labels = []
...
def main():
    ...
    criterion = BalancedSoftmaxLoss(class_counts=class_counts_tensor)
    
    # Train the model
    print("Training Uniform Expert with Balanced Softmax Loss...")
    model = train_model(model, train_loader, criterion)
    
    # Evaluate the model
    print("\nEvaluating on validation data...")
    evaluate(model, val_loader, report_filename="uniform_classification_report.txt")
    
    # Save the trained model
    save_model(model, 'uniform_expert.pth')
